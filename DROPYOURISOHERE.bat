...
if not %ERRORLEVEL%==0 goto :missingPython
...
:missingPython
echo.
echo  You do not have python installed on your system (or it is missing from your user path)
echo  This utility requires python 2.7 (or later) to run.
goto exit
...
python obtain_patch_info.py || goto :failedObtainPatches
...
python read_and_apply_patch_info.py "%originalISO%" "%certUtilExists%" || goto :failedApply
